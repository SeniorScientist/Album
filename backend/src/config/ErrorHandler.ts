...
export class ErrorHandler {
  ...
  public static normalizeError(error: ApiError): ApiError {
    const normalizedError: ApiError = new ApiError(error)
    Object.keys(constants.errorMap).forEach(errorKey => {
      if (errorKey === normalizedError.name) Object.assign(normalizedError, constants.errorMap[errorKey as keyof typeof constants.errorMap])
    })
    Object.keys(constants.errorTypes).forEach(errorTypeKey => {
      const errorType = constants.errorTypes[errorTypeKey as keyof typeof constants.errorTypes]
